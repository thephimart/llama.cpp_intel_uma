...
if (Test-Path $ConfigPath) {
    $BaseConfig = Get-ChildItem $ConfigPath -Filter "*.cfg" |
        Where-Object { $_.Name -match '^ZZZ-Base' } |
        Sort-Object Name |
        Select-Object -First 1
}

if ($BaseConfig) {
    Write-Host "Base config detected: $($BaseConfig.Name)`n"
} else {
    Write-Warning "No base config found (expected ZZZ-Base-*.cfg)`n"
}
...
# ---------- Summary ----------
Write-Host "`n========================================="
Write-Host "Launch configuration:"
